...
3. Edit `climbu/climbu-livescoring/config.php` and change `$homehost="192.168.0.100";` to your `LAN IP ADDRESS`. (on 1st run change only this variable)
...
Install o Visual C++ Redistributable for Visual Studio 2015 - http://www.microsoft.com/en-us/download/details.aspx?id=48145
...
|_config.php //config file, take a look and change w/ u need..
 |_data
  |_climbu-livescoring.dat //sqlite3 database
 |_langs
  |_pt_BR //to translate.. clone this directory to your language code open .po file w/ poedit and set per-user language at database or change main language at config.php
   |_LC_MESSAGES
    |_default.po //poedit file (use poedit to translate)
    |_default.mo 
...
..if u have any doubt, just download my windows_x86 portable version and take a look at my BATs.
```
